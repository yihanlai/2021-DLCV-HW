...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, pos_embed, tgt, tgt_mask):
        ...
        # decoder
        hs, att_map = self.decoder(tgt, memory, memory_key_padding_mask=mask, tgt_key_padding_mask=tgt_mask,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_pre(self, tgt, memory,
        ...
        # print(score.shape)
        enc_image_size = (int(score.shape[2]/19), 19)
        att_matrix = score[0].detach().cpu().numpy()
        att_map = att_matrix.reshape((128, enc_image_size[0], enc_image_size[1]))
        ########################################################################
        # q = self.with_pos_embed(tgt2, query_pos).permute(1,0,2)[0]
        # kT = self.with_pos_embed(memory, pos).permute(1,2,0)[0]
        # print('q', q.shape)
        # print('kT',kT.shape)
        # print('v', memory.shape)
        # enc_image_size = (int(kT.shape[1]/19), 19)
        # att_matrix = np.matmul(q.detach().cpu().numpy(), kT.detach().cpu().numpy())
        # print("attention matrix:", att_matrix.shape)
        # att_map = att_matrix.reshape((128, enc_image_size[0], enc_image_size[1]))
