...
from model import Convnet, euclidean_dist
...
if __name__=='__main__':
    ################################################
    weight = 'model_best_Euclidean.pth'
    ################################################
    args = parse_args()
    ...
    query_list = [f'query{i}' for i in range(args.N_query * args.N_way)]
    output = {}
    for i in range(len(prediction_results)):
        output[i]=prediction_results[i]

    result = pd.DataFrame.from_dict(output).transpose()
    result.columns=query_list
    result.index.name='episode_id'
